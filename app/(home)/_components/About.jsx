...
export default function About() {
  ...
  useEffect(() => {
    // Initialize GSAP timeline for the timer animation
  ...
  useEffect(() => {
    ...
    const currentFactNumber = parseInt(
      facts[currentFactIndex].number.replace(/[^\d]/g, "")
    ); // Extract numeric part
    const currentFactLetter = facts[currentFactIndex].number.replace(/\d/g, ""); // Extract letter part
  
    const previousFactNumber = parseInt(
      facts[
        (currentFactIndex - 1 + facts.length) % facts.length
      ].number.replace(/[^\d]/g, "")
    );
  
    if (factNumberRef.current) {
      gsap.fromTo(
        factNumberRef.current,
        { textContent: previousFactNumber }, // Start from the previous number
        {
          textContent: currentFactNumber, // End at the current number
          duration: 1, // duration for counting animation
          ease: "power2.out", // easing
          snap: { textContent: 1 }, // snap to integer values
          onUpdate: function () {
            if (factNumberRef.current) {
              factNumberRef.current.innerHTML =
                Math.round(this.targets()[0].textContent).toString() +
                currentFactLetter; // Append the letter
            }
          },
        }
      );
    ...
    if (timerRef.current && timelineRef.current) {
      ...
      timelineRef.current
        ...
        .to(timerRef.current, {
          ...
          onComplete: () => {
            // Call handleNext when the animation completes
            handleNext();
          },
        });
    }
